...
void Chapter11Sample02::Initialize() {
...
void Chapter11Sample02::Update(float deltaTime) {
    ...
    if (mShowMeshes) {
        ...
        for (int i = 0, size = mPosePalette.size(); i < size; ++i) {
...
void Chapter11Sample02::Render(float inAspectRatio) {
    ...
    if (mShowMeshes) {
        ...
        if (mSkinType == SkinningType::GPU) {
            Uniform<mat4>::Set(shader->GetUniform("animated"), mPosePalette);
        }
...
void Chapter11Sample02::Shutdown() {
...
void Chapter11Sample02::ImGui(nk_context* ctx) {
    nk_begin(ctx, "Chapter 11, Sample 02", nk_rect(5.0f, 5.0f, 300.0f, 190.0f), NK_WINDOW_BORDER | NK_WINDOW_NO_SCROLLBAR);
    ...
    show = (int)mShowMeshes;
    if (nk_checkbox_label(ctx, "Show Skinned Meshes", &show)) {
        mShowMeshes = (bool)show;
    }
