...
void Chapter12Sample02::Initialize() {
	...
	mCrossfadingController.SetSkeleton(mSkeleton);
	mCrossfadingController.Play(&mClips[0]);
...
void Chapter12Sample02::Update(float deltaTime) {
	mCrossfadingController.Update(deltaTime);
	mCurrentPoseVisual->FromPose(mCrossfadingController.GetCurrentPose());

	if (mShowMesh) {
		if (mSkinType == SkinningType::CPU) {
			for (int i = 0, size = mMeshes.size(); i < size; ++i) {
				mMeshes[i].CPUSkin(mSkeleton, mCrossfadingController.GetCurrentPose());
			}
		}
		else if (mSkinType == SkinningType::GPU) {
			mCrossfadingController.GetCurrentPose().GetMatrixPalette(mPosePalette);
		}
	}
}
...
void Chapter12Sample02::ImGui(nk_context* ctx) {
	...
	int selected = nk_combo(ctx, (const char**)mUIClipNames, (int)mNumUIClips, (int)mNextAnim, 25, nk_vec2(200, 200));
	...
	if (nk_button_label(ctx, "Cross Fade")) {
		mCrossfadingController.FadeTo(&mClips[mNextAnim], mFadeTime);
	}
