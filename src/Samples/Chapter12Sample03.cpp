...
void Chapter12Sample03::Initialize() {
...
void Chapter12Sample03::Update(float deltaTime) {
	mCurrentPose = mAddPose = mSkeleton.GetRestPose();
...
void Chapter12Sample03::Render(float inAspectRatio) {
...
void Chapter12Sample03::Shutdown() {
...
void Chapter12Sample03::ImGui(nk_context* ctx) {
	nk_begin(ctx, "Chapter 13, Sample 01", nk_rect(5.0f, 5.0f, 300.0f, 225.0f), NK_WINDOW_BORDER | NK_WINDOW_NO_SCROLLBAR);
	...
	nk_label(ctx, "Additive:", NK_TEXT_LEFT);
	selected = nk_combo(ctx, (const char**)mUIClipNames, mNumUIClips, (int)mAdditiveIndex, 25, nk_vec2(200, 200));
	if ((unsigned int)selected != mAdditiveIndex) {
		mClips[mAdditiveIndex].SetLooping(true);
		mAdditiveIndex = (unsigned int)selected;
		mAdditiveBase = MakeAdditivePose(mSkeleton, mClips[mAdditiveIndex]);
		mClips[mAdditiveIndex].SetLooping(false);
	}
	nk_label(ctx, "Strength:", NK_TEXT_LEFT); 
	nk_slider_float(ctx, 0.0f, &mAdditiveTime, 1.0f, 0.001f);
